    ...
    async def create_agent(self, config: Dict[str, Any]) -> Agent:
        ...
        if calendar:
            instructions += (
                ...
                "1. ASK FOR DAY FIRST, THEN TIME: Always ask what day works first. Then ask about morning/afternoon/evening if needed. Never ask both in one robotic compound question.\n"
                "2. OFFER EXACTLY 3 OPTIONS: When presenting availability, offer exactly 3 slots in ONE casual sentence. Always say the day before the time (e.g., 'I’ve got Wednesday at 2, Thursday at 1:30, or Friday at 3 — any of those work?'). Do not list more than 3, do not use numbers, and do not use multiple sentences.\n"
                "3. SKIP REDUNDANT QUESTIONS: If the caller already provided information (e.g., 'Tuesday afternoon works'), move straight to checking availability for that time. Do not ask questions they've already answered.\n"
                "4. ALWAYS OFFER ALTERNATIVE: If no slots work for a specific day/time, never just say 'Nothing is available'. Instead, say: 'No worries — want me to look at another day, or maybe check mornings that day?' Redirect and keep momentum.\n"
                "5. SHORT CONFIRMATION: After booking, use exactly one short, confident sentence: 'You’re all set for [Day] at [Time]. You’ll get a confirmation text shortly.' Do not over-explain or repeat details.\n\n"
