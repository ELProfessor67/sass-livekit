...
export const generateSpecificMockCall = (id: string, callDate: Date, callIndex: number): MockCall | null => {
  ...
  if (callIndex % 5 === 0) {
    resolution = "Scam";
    summary = `Call from ${firstName} ${lastName} was identified as scam. The caller was attempting to solicit unwanted services such as car insurance or energy savings programs.`;
    transcript = [
